...
{
    public function getPage($columnName, $sortDirection)
    {
        $conn = $this->connect();
        $response = [];
        $count = '';
...
// отримання даних для поточної сторінки
        $sql = "SELECT * FROM `messages` ORDER BY ". $columnName ." ". $sortDirection ." LIMIT :limit OFFSET :offset ";
...
$dataAll = $message->getPage($columnName, $sortDirection);
echo($dataAll);
